...
def train(model, optimizer, lr_scheduler, train_loader, criterion, device=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        c = challenge_metric(to_np(sigmoid(output), device), target)
        cc += c
        Loss += loss
        total += target.size(0)
        batchs += 1
...
def test(model, data_loader, device=None):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(data_loader):
            ...
            data, target = data.to(device), target.to(device)
            outputs = model(data)
            c = challenge_metric(to_np(sigmoid(outputs),device), target)
            cc += c
            batchs += 1
    return cc / batchs
...
def train_challenge2020(config):
    ...
    kernel_sizes_0 = config.get("kernel_sizes_0")
    kernel_sizes = [kernel_sizes_0, 2*kernel_sizes_0+1, 2*(2*kernel_sizes_0+1)+1]

    print("****************************************************")
    print(kernel_sizes)

    model = InceptionTimeV2(in_channels, num_classes, config['n_filters'], kernel_sizes, config['bottleneck_channels']).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--smoke-test", default=False, action="store_true", help="Finish quickly for testing")
    parser.add_argument(
        "--ray-address",
    ...
    analysis = tune.run(
        train_challenge2020,
        name="Exp_Challenge2020_Inception",
        scheduler=sched,
        stop={
        ...
        config={
            ...
            'n_filters': tune.choice([32]),
            'kernel_sizes_0': tune.choice([9, 12]),
            'bottleneck_channels': tune.choice([16, 32, 64])
