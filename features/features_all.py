...
def GetAllFeature_test( patient_info,short_table0,short_table1,short_table2,
    ...
    for each_feature in all_feature_name_list:
        print(each_feature,len(eval(each_feature)[0]))
        final_list.extend(eval(each_feature)[0])
    # hhc_feature = pd.DataFrame(data = final_array)
    # hhc_feature.to_csv(result_name+'_OUT_FEATURE.csv', sep=',', header=True, index=False)
    print('feature length',len(final_list))

    if len(final_list) != 6233:
        f = open('bad_length'+patient_info[0]+'.csv', 'w', encoding='utf-8', newline='')
    return [final_list]
...
def get_all_feature_multi(result):
    f = open('/DATASET/challenge2020_Feature/Out_10_interval_percentile_'+result[0][0][0]+'.csv', 'w', encoding='utf-8', newline='')
