...
def get_short_stat_wave_feature_from_avR(table, pid_list, long_pid_list):
    ...
    print('extract GetShortStatWaveFeature_avR begin')
    ...
    for pid in long_pid_list:
        if pid in pid_short_dic.keys() and len(pid_short_dic[pid]) - 2 > 0:
            ...
            for i in range(len(sub_table) - 2):
                ...
                T_peak = min(T_wave)
                P_peak = min(P_wave)
                Q_peak = max(prev_ts[-6:])
                R_peak = ts[0]
                S_peak = max(ts[:6])

                T_loc = np.argmin(T_wave)
                P_loc = np.argmin(P_wave)
                Q_loc = -np.argmax(prev_ts[-6:])
                R_loc = 0
                S_loc = np.argmax(ts[:6])
