...
class SplAtConv1d(Module):
    ...
    def __init__(self, in_channels, channels, kernel_size, stride=(1), padding=(0),
        ...
        # print(stride)
        self.conv = MetaConv1d(in_channels, channels*radix, kernel_size, stride, padding, dilation,
                               groups=groups*radix, bias=bias, **kwargs)
        self.use_bn = norm_layer is not None
        if self.use_bn:
            self.bn0 = norm_layer(channels*radix)
        self.relu = ReLU(inplace=True)
        self.fc1 = MetaConv1d(channels, inter_channels, 1, groups=self.cardinality)
        if self.use_bn:
            self.bn1 = norm_layer(inter_channels)
        self.fc2 = MetaConv1d(inter_channels, channels*radix, 1, groups=self.cardinality)
...
class Bottleneck(MetaModule):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        self.conv1 = MetaConv1d(inplanes, group_width, kernel_size=1, bias=False)
        ...
        if radix >= 1:
            ...
            self.conv2 = MetaConv1d(
                group_width, group_width, kernel_size=kernel_size, stride=stride,
                padding=(kernel_size - 1) // 2, dilation=dilation,
                groups=cardinality, bias=False)
            self.bn2 = norm_layer(group_width)

        self.conv3 = MetaConv1d(
...
class ResNet(MetaModule):
    ...
    def __init__(self, block, layers, radix=1, groups=1, bottleneck_width=64,
                 ...
                 last_gamma=False, norm_layer=MetaBatchNorm1d, kernel_size=7):
            ...
            if self.avg_down:
                if dilation == 1:
                    ...
                    down_layers.append(nn.AvgPool1d(kernel_size=1, stride=1,
                                                    ceil_mode=True, count_include_pad=False))
                down_layers.append(MetaConv1d(self.inplanes, planes * block.expansion,
...
def resnest_meta(pretrained=False, layers=[3, 4, 6, 3], radix=2, groups=1, bottleneck_width=64,
